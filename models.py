...
class SeqModel(nn.Module):
    def __init__(self, src_vocab_size, tgt_vocab_size, pad_id_src, pad_id_tgt, config=None):
        ...
        if self.config['data']['share_vocab']:
            ...
            self.tgt_embedding = nn.Embedding(self.tgt_vocab_size, self.options['emb_dim'], self.pad_id_tgt)
        ...
        if self.options['encoder'] == 'lstm':
            ...
            self.ctx_bridge = nn.Linear(self.options['src_hidden_dim'], self.options['tgt_hidden_dim'])
        else:
    ...
    def forward(self, input_src, input_tgt, srcmask, srclens, input_attr, attrlens, attrmask):
        ...
        # src_outputs = [batch, max_len, hidden_dim]
        # src_h_t, src_c_t = [num_layers*2, batch, hidden_dim/2]
